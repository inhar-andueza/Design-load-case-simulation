function [] = MExtremes_DLC1_5( Y,SimulationDirectory,mef,sp  )
%%
...
for j=sp.j:length(Y(:,1))
    ...
    if j==1        
        ...
        fseek(fid,0,'cof');
        fprintf(fid,'%1.0f  %s\n',n/2,'1.35  1.3   1.5   1.7  0  "DLC 1.5"         (NumDLCfiles, PSF1, PSF2, PSF3, PSF4, BinModel, DLC_Name)');
        for k=1:n
            if Z(k,8)=='N'
                h=strrep(Y(j,:),' ','');
                InputFile=['.\',h,'\',Z(k,:)];
    ...
    if j==2
        ...
        fseek(fid,0,'cof');
        fprintf(fid,'%1.0f  %s\n',n,'1.35  1.3   1.5   1.7  0  "DLC 1.5"         (NumDLCfiles, PSF1, PSF2, PSF3, PSF4, BinModel, DLC_Name)');
        for k=1:n
            h=strrep(Y(j,:),' ','');
            InputFile=['.\',h,'\',Z(k,:)];
            fprintf(fid,'"%s"\n',InputFile);
