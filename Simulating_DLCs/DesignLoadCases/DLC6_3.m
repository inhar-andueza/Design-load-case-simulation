function [] = DLC6_3( SimulationDirectory,FFWindFile,SimulationControl,InitialConditions,UnchangedPrimary,UnchangedServoDyn,UnchangedElastoDyn,sp,PrimaryFile,RunFAST,Vr )
...
%% DLC 6.3
Vhub = Vr;
%For DLC 6.3 the design standard requires six 1-h simulations at each wind condition
fid=fopen(PrimaryFile,'rt+');
for j=1:5
    fgetl(fid);
end
fseek(fid,0,'cof');
fprintf(fid,'        %3.0f ',100); % 10 min = 600 sec + 30 sec to eliminate start-up transient behaviour
fclose(fid);              
...
for i=sp.i:length(yaw)
    ...
    cd('Simulating_DLCs\Results\DLC6_3')
    mkdir EWM
    cd(SimulationDirectory)
    %%
    for k=sp.k:6
        ...
        t=[u,'.outb'];
        movefile(t,y)
        x='Simulating_DLCs\Results\DLC6_3\EWM';
        movefile(y,x)
    end
    cd('Simulating_DLCs\Results\DLC6_3')
